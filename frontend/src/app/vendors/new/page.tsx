...
export default function NewVendorPage() {
  ...
  const createMutation = useMutation({
    mutationFn: (data: Partial<Entity>) => entitiesApi.create(data),
    onSuccess: (entity) => {
    ...
    <AuthenticatedLayout>
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-4">
          <Link href="/vendors" className="p-2 rounded-lg hover:bg-gray-100">
            <ArrowLeft className="h-5 w-5 text-gray-500" />
          </Link>
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Add New Vendor</h1>
            <p className="text-gray-500 mt-1">
              Enter the vendor details and compliance requirements
            </p>
          </div>
        </div>
